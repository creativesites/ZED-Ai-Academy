  ...
  return (
    <div className="bg-slate-50 min-h-screen">
      ...
      <div className="py-12">
        ...
        {/* Hero Section */}
        <section className="relative overflow-hidden rounded-[3rem] bg-[#062e39] p-8 md:p-16 text-white shadow-2xl">
          ...
          <div className="relative z-10 max-w-2xl">
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/10 text-white text-[10px] font-black uppercase tracking-widest border border-white/10 mb-8">
              <Sparkles className="h-3.5 w-3.5 text-yellow-400" /> Student Dashboard
            </div>
            <h1 className="text-4xl md:text-6xl font-black tracking-tight mb-6 uppercase leading-tight">
              Welcome back, <span className="text-[#fd5523]">{firstName}</span>
            </h1>
            <p className="text-white/60 text-lg font-medium leading-relaxed mb-10">
              Continue your journey at {company.name}. You have {inProgressCount} course{inProgressCount !== 1 ? "s" : ""} in progress.
            </p>
            <div className="flex flex-wrap gap-4">
              <Link
                href={`/academy/${company.slug}/classroom`}
                className="h-16 px-10 rounded-2xl bg-[#fd5523] text-white text-xs font-black uppercase tracking-widest flex items-center gap-3 hover:scale-[1.02] active:scale-95 transition-all shadow-xl shadow-[#fd5523]/20"
              >
                Enter Classroom <Layout className="h-5 w-5" />
              </Link>
            </div>
          </div>
        </section>
           ...
           {enrollments.length > 0 ? (
             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {enrollments.map((enrollment) => {
                  ...
                  return (
                    <Card key={enrollment.id} className="group bg-white rounded-[3rem] border border-slate-100 shadow-sm hover:shadow-2xl hover:border-[#fd5523]/20 transition-all overflow-hidden flex flex-col h-full">
                      <div className="aspect-[16/10] bg-slate-50 relative overflow-hidden">
                        ...
                        <div className="absolute top-4 left-4">
                           <div className="px-3 py-1 rounded-full bg-white/90 backdrop-blur-md text-[10px] font-black uppercase tracking-widest text-[#062e39] shadow-lg">
                             {course.category || "General"}
                           </div>
                        </div>
                      </div>
                      <CardContent className="p-8 flex flex-col flex-1">
                        <h3 className="text-xl font-black text-[#062e39] leading-snug group-hover:text-[#fd5523] transition-colors mb-4">{course.title}</h3>
                        
                        <div className="mt-auto space-y-6">
                           {isPending ? (
                             <div className="p-4 rounded-2xl bg-amber-50 border border-amber-100 flex flex-col gap-3">
                                <p className="text-[10px] font-black uppercase tracking-widest text-amber-700">Awaiting Payment Confirmation</p>
                                <a 
                                  href={`https://wa.me/260979046745?text=Hi, I have enrolled in ${course.title} and would like to confirm my payment.`}
                                  target="_blank"
                                  className="h-10 rounded-xl bg-[#25D366] text-white text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2"
                                >
                                  Confirm on WhatsApp
                                </a>
                             </div>
                             ...
                             <div className="flex items-center justify-between">
                                ...
                                <Link 
                                  ...
                                  className="h-10 px-6 rounded-xl bg-[#062e39] text-white text-[10px] font-black uppercase tracking-widest hover:bg-[#fd5523] transition-all flex items-center gap-2"
                                >
